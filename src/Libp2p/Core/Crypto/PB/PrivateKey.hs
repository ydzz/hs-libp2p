...
module Libp2p.Core.Crypto.PB.PrivateKey (PrivateKey(..)) where
import Prelude ((+), (/))
...
data PrivateKey = PrivateKey{type' :: !(Libp2p.Core.Crypto.PB.KeyType), data' :: !(P'.ByteString)}
...
instance P'.Wire PrivateKey where
  ...
  wirePutWithSize ft' self'@(PrivateKey x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> put'FieldsSized
